import { ScriptScene, CharacterBible, VisualMedium, StylePreset } from './types';
...
`;
  ...
  // Try Gemini Models in order of capability
  const modelsToTry = [
    'gemini-2.5-flash',
    'gemini-2.0-flash',
    'gemini-1.5-flash',
  ...
  for (const model of modelsToTry) {
    try {
      ...
      const scenes: ScriptScene[] = parsed.map((item, idx) => {
        ...
        const prompts = buildVisualPrompts({
          ...
          charactersInScene: activeSceneChars,
        ...
        return {
          ...
          characterIds: activeSceneChars.map((c) => c.id),
          visualMedium: visualMedium as VisualMedium,
          stylePreset: stylePreset as StylePreset,
