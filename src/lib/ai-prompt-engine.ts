...
export function buildVisualPrompts(params: PromptGenerationParams): {
  ...
  switch (genre) {
    ...
    case 'epic_fantasy':
      if (worldCulture === 'thai' || !worldCulture) {
        genreFlavorTh = 'ฉากหลักตามเนื้อเรื่อง: บรรยากาศภาพยนตร์ไทย (ทุ่งนา, ชนบท, บ้านทรงไทย, วัดไทย, กรุงเทพ, ตลาด, หรือสถานที่ที่สอดคล้องกับเนื้อเรื่อง), แสงธรรมชาติสวยงาม, บรรยากาศอบอุ่นและสมจริง';
        ...
        genreFlavorTh = 'ยอดเขาหมอกโบราณ, ปราสาทจีนยุคโบราณ, สัตว์มังกรในตำนาน, ท้องฟ้าพายุแห่งโลกแฟนตาซีตะวันออก';
  ...
  // 4. สรุปการกระทำในฉาก
  const cleanSummary = sceneTitle.replace(/ฉากที่ \d+[:\s]*/, '').substring(0, 200);
  ...
  const imagePrompt = `${styleKeywordsTh}, ${characterDescTh}${sameFrameThPart}, กำลังทำ [${cleanSummary}${narrationContext ? ` — ${narrationContext}` : ''}], ฉากหลัง: ${genreFlavorTh}, มุมกล้อง: ${cameraMovement}, แสงเงา: ${lighting}, ภาพ${arLabelTh} คมชัดระดับ 8K ละเอียดประณีต`;
...
Aspect Ratio: ${aspectRatio}`;
  ...
  const videoMotionPrompt = `[ฉากที่ ${sceneNumber}] [มุมกล้อง: ${cameraMovement}, เคลื่อนไหวลื่นไหลแบบภาพยนตร์] [การกระทำ: ${cleanSummary}${narrationContext ? ` — ${narrationContext}` : ''}, แอ็กชันต่อเนื่องไม่ตัดข้าม]${sameFrameVideoTh} [แสงเงา: ${lighting}] [สัดส่วน: ${aspectRatio}] [สไตล์: ${videoStyleLabelTh}] รักษาความต่อเนื่องของใบหน้า ทรงผม เสื้อผ้า และฉากอย่างแม่นยำ คมชัดระดับ 4K 60fps ต่อเนื่องเนียนตา`;
